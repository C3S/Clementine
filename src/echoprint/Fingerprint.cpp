...
uint Fingerprint::adaptiveOnsets(int ttarg, matrix_u&out, uint*&onset_counter_for_band) {
    ...
    float ham[8];
    for(int i = 0 ; i != nsm ; i++)
        ham[i] = (float)(.5 - .5*cos( (2.*M_PI/(nsm-1))*i));

    int nc = (int)(floor((float)E.size2()/(float)hop)-(floor((float)nsm/(float)hop)-1));
...
uint Fingerprint::quantized_time_for_frame_delta(uint frame_delta) {
    ...
    return (uint)(((int)floor((time_for_frame_delta * 1000.0) / (float)QUANTIZE_DT_S) * QUANTIZE_DT_S) / floor(QUANTIZE_DT_S*1000.0));
}
...
uint Fingerprint::quantized_time_for_frame_absolute(uint frame) {
    ...
    return (uint)(((int)rint((time_for_frame * 1000.0) /  (float)QUANTIZE_A_S) * QUANTIZE_A_S) / floor(QUANTIZE_A_S*1000.0));
}
